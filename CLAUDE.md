...
| POST | `/api/shop/live-tryon/token` | No | Consume one live session, returns a capped Decart client token |
...
- `LIVE_TRYON_SESSION_SECONDS` / `LIVE_TRYON_MEMBER_SECONDS` — streaming cap per session, guest / member (default: `45` / `120`)
- `LIVE_TRYON_DAILY_SESSIONS` / `LIVE_TRYON_MEMBER_SESSIONS` — sessions per day per guest IP / per member (default: `3` / `10`, `0` = unlimited)
