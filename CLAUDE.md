...
- **Storefront star ratings:** customers don't review; `handlers/rating.go` derives `Product.Rating`/`RatingCount` (not persisted, `gorm:"-"`) from real units sold on non-cancelled orders — deterministic, always 4.5–5.0 (`4.5 + 0.5·u/(u+8)` + tiny per-id jitter, one decimal; ~8 units → 4.8, ~30 → 4.9, ~90 → 5.0), count = units sold. Attached by the public shop handlers (`/api/shop/products`, `/:id`, `/suggest`) only; 0/0 when never sold and the storefront `Rating` component hides the stars. Shown on `ProductCard` (under the price) and the product page (right of the price).
